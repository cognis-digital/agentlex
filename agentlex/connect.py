"""Native cognis-connect emit for agentlex — forward findings to any platform.

Maps agentlex's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    agentlex ... --format json | agentlex-emit --to stix
    agentlex-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a agentlex record toward the Finding contract before normalize().
